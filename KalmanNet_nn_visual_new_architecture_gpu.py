...
class KalmanNetNN(torch.nn.Module):
    ...
    def Build(self, SysModel,H_latent_space_learned, model_encoder, H_latent_space_fix, fix_H_flag, fix_encoder_flag):

        self.fix_H_flag = fix_H_flag
        self.fix_encoder_flag = fix_encoder_flag

        self.InitSystemDynamics(F, H_latent_space_learned, model_encoder, H_latent_space_fix, infoString="partialInfo")
    ...
    ##################################
    def InitSystemDynamics(self, F, h_learned_lower_dimension ,model_encoder, H_matrix_fix, infoString='fullInfo'):
        ...
        self.F = F
        #self.F = F.to(self.device, non_blocking=True) # state evolution matrix
        #self.F_T = torch.transpose(F, 0, 1)
        self.m = m                  #state size (x)
        self.d = H_matrix_fix.shape[0]                # state size in latent space (z)

        ####### Set Observation Function
        #self.h = h
        #self.n = n
        #if not self.fix_H_flag:
        self.h_learned_lower_dimension = h_learned_lower_dimension.double()
        #self.H_matrix_fix = H_matrix_fix.double()
        self.n = H_matrix_fix.shape[0]                #obsevation size
        #y_size * y_size
    ...
    def InitSequence(self, M1_0, T):
        ...
        self.y_previous = self.h_learned_lower_dimension(self.m1x_posterior.reshape(1, self.m)).reshape(self.d, 1)
    ...
    def step_prior(self):
        ...
        self.m1x_prior = torch.matmul(self.F(self.m1x_posterior.double()),self.m1x_posterior.double())
        # Predict the 1-st moment of y
        self.m1y = self.h_learned_lower_dimension(self.m1x_prior.float())
    ...
    def step_KGain_est(self, y):
        ...
        self.KGain = torch.reshape(KG, (self.m, self.n)).double()
    ...
    def KNet_step(self, y):
        ...
        #print("took {}".format(time.time()-t_start))
        # Save KGain in array
        ...
        INOV = torch.matmul(self.KGain, dy.double())
