...
from config import y_size, encoded_dimention, decimation_flag
...
class SystemModel:
    def __init__(self, F, given_q, real_q, H, h, given_r, real_r, T, T_test, m, n, modelname,EKF_with_encoder_flag, prior_Q=None, prior_Sigma=None, prior_S=None):
        ####################
        ### Motion Model ###
        ####################
        ...
        if EKF_with_encoder_flag:
            self.givenR = given_r * given_r * torch.eye(encoded_dimention)
        else:
            self.givenR = given_r * given_r * torch.eye(self.n)
    ...
    def Generate_states(self, T):
        ...
        for t in range(0, T):
            ...
            ########################
            # Process Noise
            xt = torch.matmul(self.F(self.x_prev), self.x_prev)
            mean = torch.zeros([self.m])
            # if self.modelname == "pendulum":
            #     distrib = MultivariateNormal(loc=mean, covariance_matrix=Q_gen)
            #     eq = distrib.rsample()
            # else:
            eq = torch.normal(mean, self.real_q)
    ...
    def GenerateBatch(self, size, T, randomInit=False):
        ...
        for i in range(0, size):
            ...
            if decimation_flag:
                self.Generate_states(T*5)
                decimated_states = torch.empty(size=[self.m, T])
                for k in range (T*5):
                    if k % 5 == 0:
                        decimated_states[:, int(k / 5)] = self.x[:, k]
